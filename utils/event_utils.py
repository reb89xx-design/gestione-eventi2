...
def event_form(existing_id=None):
    ...
    with col2:
        ...
        selected = st.selectbox("Service assegnato", options=names, index=(names.index(next((s["name"] for s in services if s["id"] == ev.get("service_id")), "— Nessuno —")) if ev.get("service_id") else 0))
...
def events_table():
    ...
    ids = df["id"].tolist()
    if ids:
        selected = st.selectbox("Modifica evento", options=["—"] + ids)
        if selected != "—":
